...
describe('Rendering Utils', () => {
    describe('worldToScreen', () => {
        it('should transform camera position to screen center (width/2, height*0.75)', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 1, rotation: 0 };
        ...
        it('should apply camera position offset with rotation', () => {
            const camera: Camera = { pos: { x: 100, y: 50 }, zoom: 1, rotation: 0 };
            // Camera position should map to screen center
            const cameraScreen = worldToScreen({ x: 100, y: 50 }, camera, 800, 600);
            ...
            const originScreen = worldToScreen({ x: 0, y: 0 }, camera, 800, 600);
        ...
        it('should apply camera zoom scale with rotation', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 2, rotation: 0 };
        ...
        it('should return screen coordinates object', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 1.5, rotation: 0 };
        ...
        it('should handle zero zoom', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 0, rotation: 0 };
        ...
        it('should handle negative world coordinates', () => {
            ...
            const screen = worldToScreen({ x: -100, y: -200 }, camera, 800, 600);
        ...
        it('should handle fractional zoom values', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 0.5, rotation: 0 };
        ...
        it('should handle combined offset and zoom with rotation', () => {
            const camera: Camera = { pos: { x: 100, y: 200 }, zoom: 2, rotation: 0 };
            // Camera position should map to screen center
            const cameraScreen = worldToScreen({ x: 100, y: 200 }, camera, 800, 600);
            ...
            const pointScreen = worldToScreen({ x: 150, y: 250 }, camera, 800, 600);
            expect(pointScreen.x).toBeCloseTo(500, 1);
            expect(pointScreen.y).toBeCloseTo(350, 1);
        });

        it('should maintain coordinate consistency', () => {
            const camera: Camera = { pos: { x: 50, y: 75 }, zoom: 1.5, rotation: 0 };
            // Camera position should always map to screen center
            const cameraScreen = worldToScreen({ x: 50, y: 75 }, camera, 1024, 768);
        ...
        it('should handle rotation transformations', () => {
            const camera: Camera = { pos: { x: 0, y: 0 }, zoom: 1, rotation: Math.PI / 2 };
