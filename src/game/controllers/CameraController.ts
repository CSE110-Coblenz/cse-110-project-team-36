...
import {
    MAX_CAMERA_ROTATION_RATE,
    CAMERA_ALIGNMENT_THRESHOLD,
} from '../../const';
 ...
 */
function calculateEasedSpeedFactor(
    absAngularDelta: number,
    threshold: number,
): number {
    ...
    const normalizedDistance = Math.min(
        1,
        distanceFromThreshold / (maxDistance - threshold),
    );
...
export class CameraController {
    ...
    update(dt: number, playerCar: Car, track: Track): void {
        ...
        const angularDelta = this.shortestAngularDistance(
            currentRotation,
        ...
        if (shouldCorrect) {
            const speedFactor = calculateEasedSpeedFactor(
                absAngularDelta,
                CAMERA_ALIGNMENT_THRESHOLD,
            );
