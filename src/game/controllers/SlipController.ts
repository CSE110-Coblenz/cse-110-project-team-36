...
export class SlipController {
    ...
    public updateSlip(car: Car, dt: number): void {
        ...
        car.slipFactor = Math.max(0, car.slipFactor - this.physicsConfig.slipDecay * dt);
        
        // Update wobble based on slip factor
        if (car.slipFactor > 0) {
            const wobblePhase = this.physicsConfig.slipWobbleFreq * car.s * 0.1;
            car.slipWobble = car.slipFactor * this.physicsConfig.slipWobbleAmp * Math.sin(wobblePhase);
