...
export class QuestionManager {
    ...
    private previousParams: { a: number; b: number; operation: string } | null =
        null;
    ...
    generateQuestion(config?: QuestionConfig): string {
        const effectiveConfig: QuestionConfig = config
            ? { ...this.config, ...config }
            : this.config;
        ...
        const questionParams = this.generateQuestionParams(
            topic,
            difficulty,
            this.previousParams,
        );
    ...
    private getTopicFromOperation(operation: string): QuestionTopic {
        switch (operation) {
            case '+':
                return QuestionTopic.ADDITION;
            case '-':
                return QuestionTopic.SUBTRACTION;
            case '*':
                return QuestionTopic.MULTIPLICATION;
            case '/':
                return QuestionTopic.DIVISION;
            default:
                return QuestionTopic.MIXED;
        }
    }
    ...
    submitAnswer(answer: number): boolean {
         ...
         */
        const wasCorrect =
            Math.abs(answer - this.currentQuestion.correctAnswer) < 0.05;
