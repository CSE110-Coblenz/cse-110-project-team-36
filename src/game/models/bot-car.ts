...
export class BotCar extends Car {
    ...
    static generateBotStats(difficulty: number, config: BotConfig): BotStats {
        const answerSpeed = Math.max(
            0.1, config.answerSpeedBase / difficulty + gaussian(0, config.answerSpeedStdDev)
        );
        const accuracy = clamp(
            config.accuracyBase * difficulty + gaussian(0, config.accuracyStdDev),
            0,
            1
        );
        const safetyTimeThreshold = Math.max(
            0.5,
            config.safetyTimeBase * difficulty + gaussian(0, config.safetyTimeStdDev)
        );
        return {
            answerSpeed,
            accuracy,
